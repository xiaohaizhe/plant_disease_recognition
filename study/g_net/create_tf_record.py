...
def load_labels_file(filename,labels_num=1,shuffle=False):
    '''
    载入标签注释文件，json格式，每个图像信息例如{"disease_class": 1, "image_id": "62fd8bf4d53a1b94fbac16738406f10b.jpg"}
    :param filename:
    ...
    with open(filename) as f:
        ...
        for image in label_list:
            ...
            for i in range(labels_num):
                label.append(int(image['disease_class']))
                # 当前样本只有1个标签
...
def create_records(image_dir, info_file, output_record_dir, resize_height, resize_width,shuffle,log=5):
    '''
    实现将图像原始数据,label,长,宽等信息保存为record文件
    注意:读取的图像数据默认是uint8,再转为tf的字符串型BytesList保存,解析请需要根据需要转换类型
    :param image_dir:原始图像的目录
    :param info_file:输入保存图片信息的文件(image_dir+info_file构成图片的路径)
    ...
    '''
    # 加载文件,仅获取一个label
    images_list, labels_list=load_labels_file(info_file,1,shuffle)
...
#        if not image:
#            print('Err:image error',image_path)
#            continue
...
def batch_test(record_file,resize_height, resize_width):
    ...
    # 读取record函数
    tf_image,tf_label = read_records(record_file,resize_height,resize_width,type='normalization')
    image_batch, label_batch= get_batch_images(tf_image,tf_label,batch_size=4,labels_nums=61,one_hot=False,shuffle=False)
...
if __name__ == '__main__':
    ...
    log=5
    # 产生train.record文件
    image_dir='D:/plant_disease/11.14/ai_challenger_pdr2018_trainingset_20181023/AgriculturalDisease_trainingset/images/'  # 图片路径
    train_labels = 'D:/plant_disease/11.14/ai_challenger_pdr2018_train_annotations_20181021.json'  # 标签路径
    train_record_output = 'record/train.tfrecords'
    create_records(image_dir,train_labels, train_record_output, resize_height, resize_width,shuffle,log)
    train_nums=get_example_nums(train_record_output)
    print("save train example nums={}".format(train_nums))

    # 产生val.record文件
    image_dir='D:/plant_disease/11.14/ai_challenger_pdr2018_validationset_20181023/AgriculturalDisease_validationset/images'
    val_labels = 'D:/plant_disease/11.14/ai_challenger_pdr2018_validation_annotations_20181021.json'  # 标签路径
    val_record_output = 'record/val.tfrecords'
